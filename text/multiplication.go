...
func (t *Text) Multiplication_conditions() {
	...
	if number_2 > 10 {
		panic("oКалькулятор должен принимать на вход числа от 1 до 10 включительно, не более!")
	}
	...
	if strings.Contains(t.Content, "\" * ") == true {
		if strings.Count(t.Content, "\"") == 2 && strings.Count(t.Content, "\" * ") == 1 {
			...
			if dlina_Y != dlina_X {
				panic("iМножитель введет неправильно! Первым аргументом выражения, подаваемым на вход, должна быть строка выделенная двойными кавычками.")
			}
...
func (t *Text) multiplication() {
	...
	//удаляем из строки те части, которые мы не должны повторять
	reg = regexp.MustCompile("[^a-zA-Z!?]+")
	t.Content = reg.ReplaceAllString(t.Content, "")

	t.Content = strings.Repeat(t.Content, nestr)
	fmt.Printf("%q", t.Content)
